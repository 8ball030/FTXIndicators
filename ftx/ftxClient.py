...
class FtxClient:
    ...
    def _sign_request(self, request: Request) -> None:
        ...
        if self._subaccount_name:
            # print("Processing for subaccount", self._subaccount_name)
...
class FTXHelper(FtxClient):
    def fetch_ohlcs(self, params):
        """
        dictionary :
        ...
        return self._get(path="/markets/{market}/candles?resolution={resolution}&limit={limit}&start_time={start_time}&end_time={end_time}".format(**params))

    def collect_dataframe_market(self, start_date: str = parser.parse("2020/07/21"),
                                 market: str = "FTT-USD",
                                 resolution: int = 60,
                                 final_end: str = parser.parse("2020/07/28")
                                 ):

        """
        Scraping historic OHLC from FTX

        """
        start_time = int(start_date.timestamp())
        end_time = int(final_end.timestamp())
        params = {"resolution": 3600,
                 ...
                 "market": market}
        results = []
        response = self.fetch_ohlcs(params)
        results += response
        last_date = response[-1]["time"]
        calls=0
        while True:
            if calls > 50: break# is there sis a calling issue somewhere in here.
            params["end_time"] = parser.parse(response[0]["startTime"]).timestamp() #- 358020 )
            final_end = parser.parse(response[0]["startTime"]).timestamp()
            response = self.fetch_ohlcs(params)
            results += response
            calls +=1
            if parser.parse(response[0]["startTime"], ignoretz=True) <= start_date: break
        return pd.DataFrame(reversed(results))

    def __init__(self, api_key=None, api_secret=None, subaccount_name=None, otp=None) -> None:
        self._session = Session()
        self._api_key = api_key
        self._api_secret = api_secret
        self._subaccount_name = subaccount_name
    ...
    def withdrawl(self, *, coin: str,
        ...
        """
        Withdraws to an address;
        {
          "coin": "USDTBEAR",
          "size": 20.2,
          "address": "Addressxxxxxxxxxxxx",
          "tag": null,
        ...
        params = {
                  ...
                  "address": address,
                  "tag": tag,
                  "code": pyotp.TOTP(self.otp).now()
        }
        self._post("wallet/withdrawals", params)
